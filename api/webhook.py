...
def find_best_recipe(text):
    ...
    if any(word in text for word in ['sert', 'güçlü', 'uyandır', 'kafein']):
        if 'çok' in text or 'ölü' in text: return 'dead_eye'
        return 'ristretto'
...
def get_espresso_menu():
    keyboard = [
        ...
        [InlineKeyboardButton("Daha Fazla ➕", callback_data='more_espresso')],
        [InlineKeyboardButton("⬅️ Ana Menü", callback_data='menu')]
    ]
    return InlineKeyboardMarkup(keyboard)

async def start(update, context):
    await update.message.reply_text("👋 Selam! Ben Barista Koçu.\nBana bir kahve adı yazabilir (örn: 'Cortado nasıl yapılır?') veya menüden seçebilirsin.", reply_markup=get_main_menu())
...
async def handle_text(update, context):
    ...
    if recipe_id:
        ...
        await update.message.reply_text("Bunu tam anlayamadım ama menüden seçebilirsin ya da başka bir kahve sorabilirsin!", reply_markup=get_main_menu())
