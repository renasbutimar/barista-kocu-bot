...
async def start(update, context):
    await update.message.reply_text("Merhaba! Ben Barista Koçun. 😊\n\nKahve dünyasını keşfetmeye hazır mısın?", reply_markup=get_main_menu())
...
async def handle_callback(update, context):
    ...
    if data == 'menu':
        ...
        await query.edit_message_text(RECIPES[data], reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Ana Menü", callback_data='menu')]]), parse_mode='Markdown')

async def handle_text(update, context):
    user_text = update.message.text
    recipe_id = find_best_recipe(user_text)
    if recipe_id:
        await update.message.reply_text(RECIPES[recipe_id], parse_mode='Markdown')
    else:
        await update.message.reply_text("Anlayamadım ama menüden seçebilirsin! 👇", reply_markup=get_main_menu())
